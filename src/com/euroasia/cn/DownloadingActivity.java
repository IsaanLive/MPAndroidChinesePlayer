...
public class DownloadingActivity extends ListActivity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if(totaldownloading >= 1){
        	text.setText("下载总次数： "+totaldownloading);
        }else{
        	text.setText("下载总次数： 0");
        }
		...
		public View getView(int position, View convertView, ViewGroup parent){
	    	...
	    	if(items.get(position).third == 100){
	    		...
	    		if(a == 2){
		    		...
		    		if(a == 1){
		        		progress.setText("等待下载");
		    		}else if(a == 0){
		    			progress.setText("完成下载");
		    		}else if(a == -1){
		        		progress.setText("失败的下载");
		    		}
	    		}
	...
	protected void onListItemClick(ListView l, View v, int position, final long id) {
		...
		if(items.get(position).third != 100){
			...
			if(downloaded == -1){
				...
				final CharSequence[] items = {"打开MP3", "设置为铃声", "重新下载", "删除", getString(R.string.btn_cancel_name)};
    ...
    public void refreshlist(){
    	...
    	if(a.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102109, "正在下载中", 100));
    		a.moveToFirst();
    	...
    	if(a1.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102110, "等待下载", 100));
    	...
    	if(a11.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102111, "已完成下载", 100));
    	...
    	if(a111.getCount() != 0){
    		adapter.addAdapterItem(new AdapterItem(102112, "失败的下载", 100));
