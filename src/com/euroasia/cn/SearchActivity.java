		...
		protected void onPostExecute(Void result) {
			if(ia == 0 && fail == 0){
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					failresults.setText("这现像似乎是您没有有效的互联网链接或较微弱信号的链接。请打开您的wifi或数据链接，然后再试一次。");
		        }else{
					failresults.setText("结果找不到");
		        }
			}else if(ia == 0 && fail > 0){
				fail = 0;
		        ...
		        if(!isInternetConnectionActive(getApplicationContext())) {
					...
					failresults.setText("这似乎是一个试图搜索的错误。请再试一次!");
		        }
			}else{
				setListAdapter(adapter);
			}
